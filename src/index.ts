import { GameAnimatedImage, GameCenterText, GameCircle, GameImage, GamePolygon, GameRect, GameRectButton } from "./engine/canvasObject.js";
...
class Robot extends GameAnimatedImage {
    ...
    constructor(game: Game) {
        super("swing.png", 16, new Vector(0, 28).add(
            new Vector(Math.random() * 100, Math.random() * 216)), 10, game, false);
    }
...
function switchEngine(g: Game, btn: GameRectButton) {
...
a.startFunc = () => {
    ...
    let btn = new GameRectButton(new Vector(8, 272), new Vector(80, 40), a,
    () => {switchEngine(a, btn)}, 'canvas', colors.base);
    btn.hoverColor = colors.highlight;
    let btn2 = new GameRectButton(new Vector(96, 272), new Vector(56, 16), a,
    () => {generateBS(a)}, '+', colors.base);
